...
const ScoringByDistance: React.FC<propsData> = ({ pars, scores, approachdistances }) => {
  ...
  useEffect(() => {
    ...
    let shotsFrom2550 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 25, 49)
    let shotsFrom5075 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 50, 74)
    let shotsFrom75100 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 75, 99)
    let shotsFrom100125 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 100, 124)
    let shotsFrom125150 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 125, 149)
    let shotsFrom150175 = countShotsFromForChart(parstot, scorestot, approachdistancestot, 150, 174)
  ...
  const countShotsFromForChart = (pars: number[], scores: string[], approachData: string[], mindistance: number, maxdistance: number): string[] => {
    ...
    approachData.forEach((value: string, index: number) => {
      if (parseInt(value) <= maxdistance && parseInt(value) >= mindistance) {
        ...
        const shotsToHole = parseInt(scores[index]) - pars[index] + 3
...
export default ScoringByDistance
