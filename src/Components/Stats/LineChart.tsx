...
const Chart: React.FC<propsData> = ({ dataArray, title, average = true, last = 7, perRound = false, Setmin = 0, Setmax = 100, fitData = true, digits = 1 }) => {
  ...
  if (average) {
    ...
    if (dataArray.length > last) {
      for (let i = last; i < dataArray.length; i++) {
        ...
        if (parseFloat(Number(average / last).toFixed(1)) > max) max = parseFloat(Number(average / last).toFixed(1))
        if (parseFloat(Number(average / last).toFixed(1)) < min) min = parseFloat(Number(average / last).toFixed(1))
        ...
        if (perRound) {
          ...
          holecount++
          if (holecount === 18) {
            const sum: number = roundarr.reduce((acc, curvalue) => acc + curvalue)
  ...
  return (
    <Box className={styling.root}>
      <Typography variant="h5" align="center" style={{ marginBottom: '4vh' }}>
        {title}
      </Typography>
