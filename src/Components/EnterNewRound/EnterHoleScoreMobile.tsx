...
const EnterHoleScoreMobile: React.FC<props> = ({ HoleNumber, Par, onSave, onClickPrev }) => {
  ...
  const [ApproachDistance, SetApproachDistance] = useState<string | undefined>(undefined)
  ...
  const handleFirEnter = (event: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  const handleFIRSelection = (event: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  const handleNextHoleClicked = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
    ...
    if (ApproachDistance && !GIR['NONE']) letDataArray.push(ApproachDistance)
    ...
    <Box className={styling.root}>
      ...
      {Par !== 3 && (
        <Box className={styling.column}>
          ...
          <RadioGroup aria-label="Fairway hit" name="fir" row onChange={handleFIRSelection} value={FIR} style={{ justifyContent: 'center' }}>
            <FormControlLabel value="left" control={<Radio />} label="Miss left" labelPlacement="top" />
            <FormControlLabel value="hit" control={<Radio />} label="Hit" labelPlacement="top" />
            <FormControlLabel value="right" control={<Radio />} label="Miss right" labelPlacement="top" />
            <TextField
      ...
      <Box className={styling.column}>
        ...
        <Grid container alignContent="center" alignItems="center" wrap="nowrap" spacing={1}>
          <FormGroup aria-label="green in regulation" row style={{ width: '100%', flexWrap: window.innerWidth < 1000 ? 'wrap' : 'nowrap' }}>
            <Grid item xs={4}>
              <FormControlLabel
                value="hit"
                control={<Checkbox onChange={handleGIRSelection} name="hit" size="small" checked={GIR['hit']} disabled={Object.keys(GIR).some((value) => GIR[value] === true && GIR['hit'] === false)} />}
                label="Hit"
                labelPlacement="top"
              />
            </Grid>{' '}
            <Grid item xs={4}>
              <FormControlLabel value="left" control={<Checkbox onChange={handleGIRSelection} name="left" size="small" checked={GIR['left']} disabled={GIR['hit'] || GIR['right'] || GIR['NONE']} />} label="Miss left" labelPlacement="top" />
            </Grid>{' '}
            <Grid item xs={4}>
              <FormControlLabel value="right" control={<Checkbox onChange={handleGIRSelection} name="right" size="small" checked={GIR['right']} disabled={GIR['hit'] || GIR['left'] || GIR['NONE']} />} label="Miss right" labelPlacement="top" />{' '}
            </Grid>{' '}
            <Grid item xs={4}>
              <FormControlLabel value="short" control={<Checkbox onChange={handleGIRSelection} size="small" name="short" checked={GIR['short']} disabled={GIR['hit'] || GIR['long'] || GIR['NONE']} />} label="Miss short" labelPlacement="top" />{' '}
            </Grid>{' '}
            <Grid item xs={4}>
              <FormControlLabel value="long" control={<Checkbox onChange={handleGIRSelection} name="long" size="small" checked={GIR['long']} disabled={GIR['hit'] || GIR['short'] || GIR['NONE']} />} label="Miss long" labelPlacement="top" />{' '}
            </Grid>{' '}
            <Grid item xs={4}>
              <FormControlLabel
                value="NONE"
                control={<Checkbox onChange={handleGIRSelection} name="NONE" size="small" checked={GIR['NONE']} disabled={Object.keys(GIR).some((value) => GIR[value] === true && GIR['NONE'] === false)} />}
                label="No try"
                labelPlacement="top"
              />{' '}
            </Grid>
      ...
      <Box className={styling.row}>
        <TextField
          ...
          style={{ fontSize: '22px', width: '20vw' }}
        ...
        <TextField
          ...
          style={{ textAlign: 'center', fontSize: '22px', width: '20vw' }}
      ...
      <Box className={styling.row}>
        <Button variant="contained" color="secondary" onClick={onClickPrev}>
          Previous Hole
        </Button>
        {HoleNumber === 18 ? (
          <Button variant="contained" color="primary" onClick={handleNextHoleClicked} startIcon={<CloudUploadIcon />}>
            Save Round
          </Button>
        ) : (
          <Button variant="contained" color="secondary" ref={inputNextHoleButton} onClick={handleNextHoleClicked}>
            Next Hole
          </Button>
        )}
      </Box>
...
export default EnterHoleScoreMobile
