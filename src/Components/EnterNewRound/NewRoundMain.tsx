...
const NewRoundMain: React.FC = () => {
  ...
  const handleTeeSelection = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
    const input = event.target as HTMLElement
      ...
      {SelectedTeeBox === '' && SelectedCourse !== '' && (
        <Box className={styling.teeboxselection}>
          ...
          <Box className={styling.columnbox}>
            <Button className={styling.teebuttonwhite} onClick={handleTeeSelection}>
              White
            </Button>
            <Button className={styling.teebuttonyellow} onClick={handleTeeSelection}>
              Yellow
            </Button>
          </Box>
        </Box>
      )}
      {data !== undefined && SelectedCourse !== '' && SelectedTeeBox !== '' && (
        <>
          {window.innerWidth > 1200 ? (
            ...
            <EnterHoleScoreMobile HoleNumber={HoleNumber} Par={data?.courseOne?.pars[HoleNumber - 1]} onSave={handleNextHoleButtonClicked} onClickPrev={handlePreviousHoleButtonClicked} />
          )}
          {window.innerWidth > 1200 ? (
            ...
            <ViewRoundMobile Coursename={SelectedCourse} Pars={data?.courseOne?.pars} Strokes={ScoreCard} Putts={Putts} Fairways={FIR} GIRs={GIR} Penalties={Penalties} FWBunkers={FairwayBunkers} GreenBunkers={GreenBunkers} />
          )}
        </>
      )}
    </Box>
