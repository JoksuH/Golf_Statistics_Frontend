...
const NewRoundCourseSelection: React.FC<props> = ({ onClick }) => {
  ...
  const handleSearchTermEnter = (event: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  return (
    <div>
      ...
      {data &&
        CourseList &&
        CourseList.map((course: CourseData, index: number) => {
          if (index < 10) return <CourseBox name={course.name} key={course._id} onClick={onClick} />
          else return <p></p>
        })}
